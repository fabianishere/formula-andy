...
public class LobbyBalancerTest {
    ...
    public void equalsDifferentLobbies() {
        assertNotEquals(new LobbyBalancer(new HashMap<ActorRef, Lobby>() {{
            put(null, new Lobby(UUID.randomUUID().toString(), LobbyStatus.PREPARATION, new LobbyConfiguration(1, Duration.ZERO, Duration.ZERO), Collections.emptySet(), Collections.emptyList()));
        }}), information);
    }
